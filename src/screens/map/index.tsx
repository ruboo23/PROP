                ...
                <>
                  ...
                  <MapView style={styles.map} customMapStyle={mapStyle} initialRegion={{ latitude: location.latitude, longitude: location.longitude, latitudeDelta: 0.0922, longitudeDelta: 0.0421, }} showsUserLocation={true} onMapReady={handleMapReady} >
                    {markers.map((marker, index) => (
                      <Marker key={'search' + index} coordinate={{ latitude: marker.latitud, longitude: marker.longitud }} title={marker.nombre} description={marker.descripcion} image={require('../../../assets/Marker.png')} onCalloutPress={(e) => { navigation.navigate('PerfilComercio', { id: marker.id, esComercioLogueado: false }) }} >
                        <Callout key={'call' + index}>
                          <SafeAreaView key={'safeArea' + index} style={styles.container}>
                            <Image key={'image' + index} source={{ uri: `https://propapi-ap58.onrender.com/api/Imagen/${marker.imagenNombre}` }} style={{ width: 22, height: 22 }} />
                            <View key={'view' + index} style={{ display: 'flex', flexDirection: 'column' }}>
                              <Text key={'text' + index} style={{ flex: 1, fontWeight: 'bold' }}>
                                {marker.nombre}
                              </Text>
                              <Text key={'text2' + index} style={{ flex: 1 }}>
                                {marker.descripcion}
                              </Text>
                              <Text key={'text3' + index} style={{ flex: 1, color: '#999' }}>
                                {marker.direccion}
                              </Text>
                            </View>
                          </SafeAreaView>
